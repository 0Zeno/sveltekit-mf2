<script lang="ts">
  ...
  import { partsToTree, type TreeNode } from './partsToTree';

  const { id, props } = $props();

  const trans = $t(id, props)
  
  const mf = new MessageFormat(trans.locale, trans.value)
  const translation = mf.formatToParts(props)
	
  const tree = partsToTree(translation)
</script>
